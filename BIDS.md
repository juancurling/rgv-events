...
- `data/bids.json` — the registry: every RGV purchasing division (cities, counties,
  ...
  solicitations, federal/state search URLs, bid platforms, community sources, and
  PlanHub setup notes.
...
1. For each entity in `data/bids.json`, check its `purchasing_url`/portal for new
   solicitations matching the six trades + general construction/CSP/JOC.
...
Paid/membership: RGV AGC plan room (Pharr/Brownsville), PlanHub (already subscribed —
see setup notes in the dashboard), CMBL ($70/yr) + free HUB certification.
